...
#[derive(Module, Debug)]
pub struct ModelNoPool<B: Backend> {
    conv2d1: Conv2d<B>,
    conv2d2: Conv2d<B>,
    conv2d3: Conv2d<B>,
    gemm1: Linear<B>,
...
impl<B: Backend> Default for ModelNoPool<B> {
    fn default() -> Self {
        Self::from_file("src/model/cnn_v2", &Default::default())
    }
}

impl<B: Backend> ModelNoPool<B> {
...
impl<B: Backend> ModelNoPool<B> {
    ...
    pub fn new(device: &B::Device) -> Self {
        ...
        let gemm1 = LinearConfig::new(15496, 256).with_bias(true).init(device);
    ...
    pub fn forward(&self, input1: Tensor<B, 4>, input2: Tensor<B, 2>) -> Tensor<B, 2> {
        ...
        let flatten1_out1 = {
            // Calculate dim based on relu3 instead of maxpool
            let leading_dim = relu3_out1.shape().dims[..1].iter().product::<usize>() as i32;
            relu3_out1.reshape::<2, _>([leading_dim, -1])
        };
