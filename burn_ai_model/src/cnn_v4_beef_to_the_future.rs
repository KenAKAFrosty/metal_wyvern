...
impl<B: Backend> ModelBeefierCnn<B> {
    ...
    pub fn new(device: &B::Device) -> Self {
        let conv2d1 = Conv2dConfig::new([8, 128], [3, 3])
            .with_stride([1, 1])
            ...
            .with_bias(true)
            .init(device);
        let conv2d2 = Conv2dConfig::new([128, 256], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d3 = Conv2dConfig::new([256, 512], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
        ...
        let meta_fc1 = LinearConfig::new(8, 64).with_bias(true).init(device);
        let meta_fc2 = LinearConfig::new(64, 128).with_bias(true).init(device);

        // Main head: 6400 (conv) + 64 (meta) = 6464
        let gemm1 = LinearConfig::new(12928, 1024).with_bias(true).init(device);
        let gemm2 = LinearConfig::new(1024, 512).with_bias(true).init(device);
        let gemm3 = LinearConfig::new(512, 4).with_bias(true).init(device);
        Self {
            conv2d1,
            conv2d2,
            conv2d3,
